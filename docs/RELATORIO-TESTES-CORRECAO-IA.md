...
> **Atualização (identificação automática do aluno):** ao enviar os cartões em lote, o sistema **lê o QR Code impresso no cartão** (que traz o aluno e a prova) e já identifica de quem é cada cartão, sem IA e sem custo. No diálogo do lote os alunos já vêm preenchidos ("Identificado pelo QR Code"). Depois de corrigir, a imagem do cartão aparece na página **Correção** (dentro da correção do aluno) e na página **Gerenciar Alunos** (coluna "Cartão resposta", com miniatura). O QR Code impresso nos cartões novos ficou maior (mais fácil de ler em foto ruim).
> Teste (sem IA, leitura simulada): 3 cartões enviados fora de ordem — os 2 de qualidade média foram identificados pelo QR (aluno e prova certos); o de **qualidade baixa** não teve o QR legível e precisou de seleção manual do aluno. Cartões antigos (impressos com o QR pequeno) podem falhar mais.
